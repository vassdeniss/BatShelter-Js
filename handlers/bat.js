...
module.exports = (req, res) => {
  ...
  if (pathname === '/bats/add-bat' && req.method === 'GET') {
    ...
    stream.on('error', (err) => handleError(res, err));
    stream.on('end', () => handleEnd(res));
  } else if (pathname.includes('/bats-edit') && req.method === 'POST') {
    const form = new formidable.IncomingForm();
    ...
    form.parse(req, (err, fields, files) => {
      ...
      fs.readFile('./data/bats.json', 'utf-8', (err, data) => {
        ...
        let bat = bats.find((bat) => bat.id === batId);
        bat.name = fields.name;
        bat.description = fields.description;
        bat.breed = fields.breed;
        bat.image = files.upload.originalFilename;
